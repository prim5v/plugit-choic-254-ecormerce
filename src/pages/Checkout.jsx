...
useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (
        response.data.mpesa_response &&
        response.data.mpesa_response.CheckoutRequestID
      ) {
        setCheckoutRequestId(response.data.mpesa_response.CheckoutRequestID);
        setPaymentStatus('Pending');
      } else {
        setError(
          response.data.message || 'Payment initiation failed. Please try again.'
        );
      }
    } catch (err) {
      setError(
  ...
  const renderPaymentStatusMessage = () => {
    ...
    if (paymentStatus === 'Paid') {
      return (
        <div className="mb-6 p-4 bg-green-100 border border-green-300 text-green-700 rounded-md flex items-center">
          ...
          <span>Payment successful! Redirecting to your orders in 5 seconds...</span>
        </div>
      );
    }
    ...
    if (paymentStatus === 'Failed') {
      return (
        <div className="mb-6 p-4 bg-red-100 border border-red-300 text-red-700 rounded-md flex items-center">
          ...
          <span>Payment was not made or failed. Please try again.</span>
        </div>
      );
    }
    ...
    if (paymentStatus === 'Pending') {
      return (
        <div className="mb-6 p-4 bg-yellow-100 border border-yellow-300 text-yellow-700 rounded-md flex items-center">
              ...
              <div className="space-y-4">
                ...
                <div className="border-t border-gray-200 pt-4 mt-4">
                  <div className="flex justify-between">
                    ...
                    <span className="text-lg font-bold text-[#8c5e3b]">${total.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
